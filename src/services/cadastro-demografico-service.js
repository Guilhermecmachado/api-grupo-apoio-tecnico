...
module.exports = {
    ...
    atualizar: (id, beneficios, beneficios_valor, curso_frequenta, estado_civil, estudou_ate, frequenta_escola, genero, grupo_etnico, idade, nome_completo, outra_fonte_renda, outra_fonte_renda_valor, pne, posicao_familiar, profissao, renda_principal, renda_principal_valor, situacao_ocupacional, cid, cancer, autismo, gestante, cadastrador_id, data_alteracao) => {
        return new Promise((aceito, rejeitado) => {
            db.query('UPDATE gta_cadastro_demografico_socio_economico SET beneficios=?, beneficios_valor=?, curso_frequenta=?, estado_civil=?, estudou_ate=?, frequenta_escola=?, genero=?, grupo_etnico=?, idade=?, nome_completo=?, outra_fonte_renda=?, outra_fonte_renda_valor=?, pne=?, posicao_familia=?, profissao=?, renda_principal=?, renda_principal_valor=?, situacao_ocupacional=?,cid=?,cancer=?, autismo=?, gestante=?,cadastrador_id=?,data_alteracao=? WHERE id = ?',
                [beneficios, beneficios_valor, curso_frequenta, estado_civil, estudou_ate, frequenta_escola, genero, grupo_etnico, idade, nome_completo, outra_fonte_renda, outra_fonte_renda_valor, pne, posicao_familiar, profissao, renda_principal, renda_principal_valor, situacao_ocupacional, cid, cancer, autismo, gestante, cadastrador_id, data_alteracao, id],
                (error, results) => {
                    if (error) { rejeitado(error); return; }
                    aceito(results);
                }
